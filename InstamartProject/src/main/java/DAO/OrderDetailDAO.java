...
public class OrderDetailDAO {
	...
	public OrderDetail create(OrderDetail order) throws ClassNotFoundException, SQLException {
		...
		PreparedStatement ps = con.prepareStatement(
				"insert into order_detail(user_id, name, contact_no, ordered_date, delivery_date, delivery_time, address_id, cost_per_month) values(?,?,?,?,?,?,?,?)");
		...
		ps.setInt(7, order.getAddress().getId());
		...
		if (order.getRentalPackageList().size() > 0) {
			for (RentalPackage pkg : order.getRentalPackageList()) {
				ps = con.prepareStatement(
						"insert into order_detail_rental_package(order_detail_id,package_id) values(?,?)");
				ps.setLong(1, order.getId());
	...
	public OrderDetail findById(Integer id) throws Exception {
		...
		ArrayList<RentalPackage> packageList = (ArrayList<RentalPackage>) new RentalPackageBO()
				.getRentalPackagesByOrder(id);
